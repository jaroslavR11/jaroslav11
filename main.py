...
async def randriver(ctx):
    ...
    await ctx.send(file=picture)
            # п     р     и    р     о     д      а   
@bot.command()
async def Environmental_pollution(ctx):
    with open(f'pollution/Environmental pollution.jpg', 'rb') as f:
        # В переменную кладем файл, который преобразуется в файл библиотеки Discord!
        picture = discord.File(f)
   # Можем передавать файл как параметр!
    await ctx.send(file=picture)
...
@bot.command()
async def randpollution(ctx):
    img_name = random.choice(os.listdir('pollution'))
    with open(f'pollution/{img_name}', 'rb') as f:
        picture = discord.File(f)
    await ctx.send(file=picture)

bot.run("MTE5MzI0MTg3MzM4MTMzOTMyNg.GvmgkY.Sjrr3Z9BCoFO6hQaprOe-JfixvUzMEZTkFkpQ8")